...
@section('title', __('Approval queue'))
...
<style>
    ...
    .kmsar-tab-badge.endorsed { background: #ECFDF5; color: #059669; }
    .kmsar-tab-badge.returned { background: #FEF2F2; color: #DC2626; }
    .kmsar-tab-badge.muted    { background: #F1F5F9; color: #94A3B8; }
    ...
    .queue-meta-row span:first-child {
        font-size: 13px;
        color: #475569;
    }
</style>
...
<div class="kmsar-page-header" style="margin-bottom:24px;">
    <div>
        <nav class="kmsar-breadcrumb" aria-label="Breadcrumb">
            <span class="kmsar-breadcrumb-current">{{ __('Approval Queue') }}</span>
        </nav>
        <div style="display:flex;align-items:center;gap:12px;margin-top:4px;flex-wrap:wrap;">
            <h1 class="kmsar-h1" style="margin:0;">{{ __('Approval Queue') }}</h1>
            @if ($pending->count() > 0)
                <span style="display:inline-flex;align-items:center;padding:4px 12px;font-size:12px;font-weight:600;border-radius:9999px;background:rgba(212,175,55,0.15);color:#B8860B;border:1px solid rgba(212,175,55,0.35);">
                    {{ $pending->count() }} {{ __('pending') }}
                </span>
            @endif
        </div>
        <p style="font-size:14px;font-weight:500;color:#0F172A;margin:6px 0 2px;">{{ auth()->user()->college?->name ?? '—' }}</p>
        <p style="font-size:13px;color:var(--color-text-muted);margin:0;">{{ __('Research awaiting your endorsement at the college level') }}</p>
    </div>
</div>
    ...
    <div class="kmsar-card-body">
        ...
        <div class="kmsar-tab-bar" role="tablist">
            <button type="button" role="tab" aria-selected="true"
                ...
                {{ __('Pending Endorsement') }}
                <span class="kmsar-tab-badge pending">{{ $pending->count() }}</span>
            </button>
            <button type="button" role="tab" aria-selected="false"
                class="kmsar-tab-btn" id="tab-endorsed"
                onclick="switchTab('endorsed')">
                {{ __('Endorsed') }}
                <span class="kmsar-tab-badge endorsed">{{ $endorsed->count() }}</span>
            </button>
            <button type="button" role="tab" aria-selected="false"
        ...
        {{-- Endorsed tab --}}
        <div class="kmsar-tab-panel" id="panel-endorsed" role="tabpanel">
            @if ($endorsed->isEmpty())
                <div class="queue-empty">
                    <div class="queue-empty-icon">📋</div>
                    <div class="queue-empty-title">{{ __('No endorsed submissions yet') }}</div>
                    <div class="queue-empty-sub">{{ __('Research you endorse will appear here.') }}</div>
                </div>
            @else
                <div class="queue-meta-row">
                    <span>{{ __(':count endorsed submissions', ['count' => $endorsed->count()]) }}</span>
                    <span>{{ __('Sorted by most recently updated') }}</span>
                </div>
                @foreach ($endorsed as $research)
                    <div class="queue-card" style="border-left:4px solid #059669;">
                        ...
                        <a href="{{ route('approval.review', $research) }}" class="queue-card-action-secondary">{{ __('View') }}</a>
                    </div>
                @endforeach
            @endif
        </div>

...
@auth
    ...
    @if($u->hasRole('super_admin') && (Route::has('admin.dashboard') || Route::has('admin.users.index') || Route::has('admin.colleges.index') || Route::has('reports.index') || Route::has('audit.index')))
        ...
        @endif
        @if(Route::has('admin.smtp-settings.edit'))
            <a href="{{ route('admin.smtp-settings.edit') }}"
               class="kmsar-nav-item {{ request()->routeIs('admin.smtp-settings.*') ? 'active' : '' }}"
               aria-label="SMTP Settings"
               @if(request()->routeIs('admin.smtp-settings.*')) aria-current="page" @endif>
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true">
        ...
        @endif
        {{-- Import Data is hidden from the sidebar for now. Routes remain registered. --}}
    @endif
    ...
    @if(Route::has('logout'))
        <div class="kmsar-sidebar-signout" style="border-top: 1px solid var(--sidebar-border); margin-top: 0.75rem; padding-top: 0.5rem;">
            <form method="POST" action="{{ route('logout') }}" id="kmsar-logout-form">
                @csrf
                <button type="button" class="kmsar-nav-item w-full text-left" aria-label="Sign out" title="Sign out" onclick="kmsarSignOut()">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true">

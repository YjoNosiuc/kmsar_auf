...
async function seedResearchViaImport(browser: Browser, filePath: string): Promise<void> {
  const context = await browser.newContext();
  const page = await context.newPage();
  await adminLogin(page);
  await page.goto('/admin/import/research');
  await uploadImport(page, filePath);
  await context.close();
}
...
test.describe('Import Data — UAT Test Suite', () => {
  ...
  test.describe('User import', () => {
    ...
    test('IMPORT-005: Upload valid user_import_valid.xlsx → 3 users imported successfully', async () => {
  ...
  test.describe('Research import', () => {
    ...
    test('IMPORT-010: Upload valid research_import_valid.xlsx → 2 research records imported', async () => {
  ...
  test.describe('Document URL import', () => {
    ...
    test('IMPORT-029: Research import with document_url — links appear in Documents tab', async ({
      ...
      expect(countResearchByTitle(TITLE_MULTI_DOCS)).toBe(1);
